...
public abstract class AbstractSceneController extends WWObjectImpl implements SceneController {
     ...
     * {@link #doResolveTopPick(gov.nasa.worldwind.render.DrawContext, java.awt.Rectangle)}. This map is used only when
     * a pick rectangle is specified. Initialized to a new HashMap.
     */
    ...
    public int repaint() {
        ...
        if (perfKeys.contains(PerformanceStatistic.TEXTURE_CACHE) || perfKeys.contains(PerformanceStatistic.ALL)) {
            if (dc.getTextureCache() != null)
                this.dc.setPerFrameStatistic(PerformanceStatistic.TEXTURE_CACHE, "Texture Cache size (Kb)",
                        this.dc.getTextureCache().getUsedCapacity() / 1000);
        }

        if (perfKeys.contains(PerformanceStatistic.JVM_HEAP) || perfKeys.contains(PerformanceStatistic.ALL)) {
            long totalMemory = Runtime.getRuntime().totalMemory();
            this.dc.setPerFrameStatistic(PerformanceStatistic.JVM_HEAP, "JVM total memory (Kb)", totalMemory / 1000);

            this.dc.setPerFrameStatistic(PerformanceStatistic.JVM_HEAP_USED, "JVM used memory (Kb)",
                    (totalMemory - Runtime.getRuntime().freeMemory()) / 1000);
        }
    ...
    protected void pick(DrawContext dc) {
        ...
        try {
            ...
            if (this.isDeepPickEnabled() && (this.lastPickedObjects.hasNonTerrainObjects()
                    || this.lastObjectsInPickRect.hasNonTerrainObjects())) {
                this.doDeepPick(dc);
            }
        } catch (Throwable e) {
    ...
    protected void drawOrderedSurfaceRenderables(DrawContext dc) {
        ...
        while (dc.getOrderedSurfaceRenderables().peek() != null) {
            try {
                ...
                Logging.logger().log(Level.WARNING, Logging.getMessage("BasicSceneController.ExceptionDuringRendering"),
                        e);
            }
        }
     ...
     * context's ordered surface renderable queue. While building the composite representation this invokes
     * {@link gov.nasa.worldwind.render.SurfaceObject#render(gov.nasa.worldwind.render.DrawContext)} in ordered
     * rendering mode. This does nothing if the ordered surface renderable queue is empty, or if it does not contain any
     * SurfaceObjects.
     * <p/>
     ...
     *
     * @param dc
     *            The drawing context containing the surface objects to build a composite representation for.
     *
     ...
     * {@link gov.nasa.worldwind.render.SurfaceObject} instances in the draw context's ordered surface renderable queue.
     * This representation was built during the preRender phase. This does nothing if the ordered surface renderable
    ...
    protected void drawCompositeSurfaceObjects(DrawContext dc) {
        ...
        int attributeMask = GL2.GL_COLOR_BUFFER_BIT // For alpha test enable, blend enable, alpha func, blend func,
                                                    // blend ref.
