 ...
 */
public abstract class AbstractShape extends WWObjectImpl implements Highlightable, OrderedRenderable, Movable, Movable2,
        ExtentHolder, GeographicExtent, Exportable, Restorable, PreRenderable, Attributable {
    ...
    /**
     * Compute the intersections of a specified line with this shape. If the shape's altitude mode is other than
     * {@link WorldWind#ABSOLUTE}, the shape's geometry is created relative to the specified terrain rather than the
     ...
     * shape on the draw context's ordered renderable list for subsequent rendering. This method is called during
     * {@link #pick(DrawContext, java.awt.Point)} and {@link #render(DrawContext)} when it's been determined that the
     * shape is likely to be visible.
     *
     * @param dc
     *            the current draw context.
     *
    ...
    /**
     * Determines whether this shape's ordered renderable state is valid and can be rendered. Called by
     * {@link #makeOrderedRenderable(DrawContext)}just prior to adding the shape to the ordered renderable list.
     *
     * @param dc
     *            the current draw context.
     *
    ...
    /**
     * Draws this shape's outline. Called immediately after calling
     * {@link #prepareToDrawOutline(DrawContext, ShapeAttributes, ShapeAttributes)}, which establishes OpenGL state for
     * lighting, blending, pick color and line attributes. Subclasses should execute the drawing commands specific to
     * the type of shape.
     * <p/>
    ...
    /**
     * Draws this shape's interior. Called immediately after calling
     * {@link #prepareToDrawInterior(DrawContext, ShapeAttributes, ShapeAttributes)}, which establishes OpenGL state for
     * lighting, blending, pick color and interior attributes. Subclasses should execute the drawing commands specific
     ...
     *
     * @param xmlWriter
     *            the export writer to write to.
     *
     * @throws IOException
     *             if an IO error occurs while writing to the output destination.
     ...
     *
     * @param altitudeMode
     *            the altitude mode. The default value is {@link WorldWind#ABSOLUTE}.
     */
     ...
     * when two or more faces of this or other filled shapes are coincident. See
     * {@link gov.nasa.worldwind.render.Offset}.
     *
    ...
    /**
     * Indicates the maximum length of time between geometry regenerations. See
     * {@link #setGeometryRegenerationInterval(int)} for the regeneration-interval's description.
     *
     ...
     *            the geometry regeneration interval, in milliseconds. The default is two seconds.
     */
     ...
     *            the reference position. May be null, in which case the first position of the outer boundary is the
     *            reference position.
     */
     ...
     * {@link #determineActiveAttributes()}. The active attributes are either the normal or highlight attributes,
     * depending on this shape's highlight flag, and incorporates default attributes for those not specified in the
     * applicable attribute set.
     *
    ...
    /**
     * Indicates whether standard lighting must be applied by consulting the current active attributes. Calls
     * {@link #mustApplyLighting(DrawContext, ShapeAttributes)}, specifying null for the activeAttrs.
     *
     * @param dc
     *            the current draw context
     *
     ...
     *            the current draw context
     * @param activeAttrs
     *            the attribute bundle to consider when determining whether lighting is applied. May be null, in which
     *            case the current active attributes are used.
     *
     * @return true if lighting must be applied, otherwise false.
     */
    ...
    /**
     * Indicates whether normal vectors must be computed by consulting the current active attributes. Calls
     * {@link #mustCreateNormals(DrawContext, ShapeAttributes)}, specifying null for the activeAttrs.
     *
     * @param dc
     *            the current draw context
     *
     ...
     *            the current draw context
     * @param activeAttrs
     *            the attribute bundle to consider when determining whether normals should be computed. May be null, in
     ...
     *
     * @param imageSource
     *            the image source for which to create the texture.
     *
    ...
    public void render(DrawContext dc) {
        ...
        // time as an OrderedRenderable. The first two calls determine whether to add the shape to the ordered
        // renderable
     ...
     *            the current draw context.
     * @param pickColor
     *            the unique color for this shape.
     *
    ...
    protected OGLStackHandler beginDrawing(DrawContext dc, int attrMask) {
        ...
        attrMask |= GL2.GL_CURRENT_BIT | GL2.GL_DEPTH_BUFFER_BIT | GL2.GL_LINE_BIT | GL2.GL_HINT_BIT // for outlines
    ...
    private void doMyRestoreState(RestorableSupport rs, RestorableSupport.StateObject context) {
        ...
        /*
         * so = rs.getStateObject(context, "highlightAttrs"); if (so != null) { ShapeAttributes attrs =
         * (this.getHighlightAttributes() != null) ? this.getHighlightAttributes() : new BasicShapeAttributes();
         * attrs.restoreState(rs, so); this.setHighlightAttributes(attrs); }
         */
    }
}
